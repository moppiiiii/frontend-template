...
description: このテンプレートに新しい Supabase テーブル/リソースの CRUD を追加するときに使う。「テーブルを足して」「リソースを追加」「<名前>の CRUD を作って」「todos と同じパターンで <名前> を作って」等で発火。schema → appSchema 合流 → serverFn → 楽観フック → route の規約を守って実装する。
---
